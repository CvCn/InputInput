...
local function assertArgument(fun_name, arg_types)
   ...
   for i, v in ipairs(arg_types) do
      ...
      if not (got_type == expected_type or optional and arg == nil) then
         LOG:Error(msg:format(i, fun_name, expected_type, got_type), 2)
      end
   end
end
...
function utf8.char(...)
   ...
   for i, v in ipairs({ ... }) do
      if type(v) ~= "number" then
         LOG:Error(("bad argument #%d to 'char' (number expected, got %s)"):format(i, type(v)))
      elseif v < 0 or v > 0x10FFFF then
         LOG:Error(("bad argument #%d to 'char' (value out of range)"):format(i))
      elseif v < 0x80 then
         -- single-byte
...
function utf8.codepoint(s, i, j)
   ...
   if i < 1 then
      LOG:Error("bad argument #2 to 'codepoint' (out of bounds)")
   end
   j = normalize_range(s, j or i)
   if j > #s then
      LOG:Error("bad argument #3 to 'codepoint' (out of bounds)")
   end
...
function utf8.len(s, i, j)
   ...
   if i < 1 or #s + 1 < i then
      return -1
      -- LOG:Error("bad argument #2 to 'len' (initial position out of bounds)")
   end
   j = normalize_range(s, j or -1)
   if j > #s then
      return -1
      -- LOG:Error("bad argument #3 to 'len' (final position out of bounds)")
   end
...
function utf8.offset(s, n, i)
   ...
   if n >= 0 then
      ...
      if i <= 0 or #s + 1 <= i then
         LOG:Error("bad argument #3 to 'offset' (position out of bounds)")
      end
   end
   ...
   elseif n > 0 then
      ...
      if not next_char(s, i) then
         LOG:Error("initial position is a continuation byte")
      end
...
function utf8.sub(str, b, e)
   local start = utf8.offset(str, b)
   local stop = next_char(str, utf8.offset(str, e + 1))
